...
SELECT * FROM (SELECT 'Watch', 'Doge') AS tmp
WHERE NOT EXISTS (
    SELECT name FROM users WHERE name = 'Watch' AND surname = 'Doge'
) LIMIT 1;

INSERT INTO users (name, surname)
SELECT * FROM (SELECT 'Artem', 'Zakharov') AS tmp
WHERE NOT EXISTS (
    SELECT name FROM users WHERE name = 'Artem' AND surname = 'Zakharov'
) LIMIT 1;
